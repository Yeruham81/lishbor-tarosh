...
function AdminDashboard() {
  ...
  const defs = useQuery({
    queryKey: ["admin", "defs", "recent"],
    queryFn: () => defsFn({ data: { limit: 5, offset: 0 } }),
  });
  const subs = useQuery({
    queryKey: ["admin", "subs", "recent"],
    queryFn: () => subsFn({ data: { status: "all", limit: 5, offset: 0 } }),
  });
  const players = useQuery({
    queryKey: ["admin", "players", "recent"],
    queryFn: () => playersFn({ data: { limit: 5, offset: 0 } }),
  });
  const lowSuccess = useQuery({
    queryKey: ["admin", "health", "low"],
    queryFn: () => healthFn({ data: { flag: "low_success_rate", limit: 200 } }),
  });
  const highDis = useQuery({
  ...
  const missingExp = useQuery({
    ...
    queryFn: () => healthFn({ data: { flag: "missing_explanation", limit: 500 } }),
  });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4 mb-6">
        <StatCard
          label="סה״כ שחקנים"
          value={fmt(k.total_players)}
          hint={`פעילים השבוע: ${fmt(k.active_players)}`}
          tone="primary"
        />
        <StatCard label="הגדרות פעילות" value={fmt(k.total_definitions)} hint={`טיוטות: ${fmt(k.draft_definitions)}`} />
        <StatCard
          label="הגדרות ממתינות לאישור"
          value={fmt(k.pending_submissions)}
          hint="הגדרות מוצעות"
          tone="warning"
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">
              <FileText className="size-4 inline ml-1" /> הגדרות אחרונות
            </CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">
              <AlertTriangle className="size-4 inline ml-1 text-amber-500" /> אירועים למעקב
            </CardTitle>
          </CardHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">
              <Users className="size-4 inline ml-1" /> שחקנים מובילים
            </CardTitle>
            <Button variant="ghost" size="sm" asChild>
